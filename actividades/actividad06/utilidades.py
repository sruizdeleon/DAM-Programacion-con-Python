...
# Pedir ociones
def pedir_opciones(instrucion: str, opciones: list):
    '''Pide y valida que se introduca una opción válida y la devuelve'''
    while True:
        texto = input(instrucion)
...
def pedir_entero(instruccion: str) -> int:
    ...
    while True:
        ...
        try:
            ...
            print("\n[ERROR] - no has proporcionado un número entero válido (Ejemplo: 1)")

# Enteros positivo
def pedir_entero_positivo(instruccion: str) -> int:
    '''
    Pide un número entero positivo al usuario hasta que se proporcione uno válido y lo devuelve.
    '''
    while True:
        texto = input(instruccion)
        try:
...
# Enteros entre rango
def pedir_entero_entre(instruccion: str, minimo: int, maximo: int) -> int:
    '''
    Pide un número entero al usuario hasta que se proporcione uno válido
    dentro del rango y lo devuelve.
    '''
    while True:
        texto = input(instruccion)
        try:
            entero = int(texto)
            if entero < minimo:
                print("\n[ERROR] - has proporcionado un número menor al mínimo")
            elif entero > maximo:
                print("\n[ERROR] - has proporcionado un número mayor al máximo")
            else:
                return entero
        except ValueError:
            print("\n[ERROR] - no has proporcionado un número entero válido (Ejemplo: 1)")
...
def pedir_decimal(instruccion: str) -> float:
    ...
    while True:
        ...
        try:
            ...
            print("\n[ERROR] - no has proporcionado un número decimal válido (Ejemplo: 1.5)")

# Decimales entre rangos
def pedir_decimal_entre(instruccion: str, minimo: float, maximo: float) -> float:
    '''
    Pide un número decimal al usuario hasta que se proporcione uno válido
    detro del rango definido y lo devuelve.
    '''
    while True:
        texto = input(instruccion)
        try:
    ...
    while True:
        ...
        try:
            ...
            if entero == 1:
                ...
                print("\n[ERROR] - no has proporcionado un valor válido (1=Sí o 0=No)")
        except ValueError:
            print("\n[ERROR] - no has proporcionado un número entero válido (1=Sí o 0=No)")
...
def pedir_string(instruccion: str) -> str:
    ...
    while True:
        ...
        try:
            ...
            print("\n[ERROR] - no has proporcionado una cadena de texto válida.")
